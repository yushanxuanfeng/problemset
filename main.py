...
def make_problem(code):
    ...
    get_problem_description(f"{prefix}/{code}{suffix}",
                            safe_open(f"./{code}/题面.md"),
                            safe_open(f"./{code}/题目说明.txt"))
    get_problem_solution(f"{prefix}/solution/{code}{suffix}",
                         safe_open(f"./{code}/题解.md"))
...
def create_template(name):
    ...
    safe_open(f"./{name}/problem.txt").write("""时间限制：1s
空间限制：512MB
知识点：""")
    safe_open(f"./{name}/std.cpp").write("""#include<bits/stdc++.h>
using namespace std;
using ll = long long;
...
def generate_template(name, zipOnly=False):
    if not zipOnly:
        generate(f"./{name}/gen.cpp", f"./{name}/std.cpp", f"./{name}/data",
                 f"./{name}/samples", numbers=range(1, 11), samples=range(1, 11))
    zip = zipfile.ZipFile(f"./{name}.zip", 'w', zipfile.ZIP_DEFLATED)
    zip.write(f"./{name}/problem.md")
    zip.write(f"./{name}/problem.txt")
